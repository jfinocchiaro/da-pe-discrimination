...
\jessiet{I think this notion of fairness allows for equity better, but I also hesitate to give a quantitative definition of equity... see the toy example I'm adding below.}
...
\subsection{A toy example: Group fairness}
This example is rather silly, since taking the same property would yield equal treatment, but there should be a distribution where this is not the case...

Consider data distributions $D^a$ and $D^d$ drawn over $\X \times \Y$ with $\X = \{e,u\}$\jessiet{university degree/educated, no university degree...?  idk} and $\Y = \{0,1\}$
Suppose we have $D^g_x = Pr[Y=1 | x, g]$ with $D^d_u = 1/3$ and $D^d_e = 2/3$, and $D^a_u = 1/5$ and $D^a_e = 4/5$.
Moreover, let $D^g[x = e] = 1/4$ for both groups.

We now have
\begin{align*}
\E_X D^d_X &= \frac 3 4 \times \frac 1 5 + \frac 1 4 \times \frac 4 5 = \frac 7 {20}\\
\E_X D^a_X &= \frac 3 4 \times \frac 1 3 + \frac 1 4 \times \frac 2 3 = \frac 5 {12}\\
\end{align*}

Taking $\Gamma^a = \Ind{p_x \geq 0.4}$ and $\Gamma_d = \Ind{p_x \geq 0.35}$ yields equalized odds since both groups have 1/4 of their members attain $\hat Y = 1$, but doesn't equalize false positive rates.
Consider
\begin{align*}
FPR(d) &= Pr[Y = 0 | \Gamma^d(p^d_x) = 1] = 1 - Pr[Y = 1 | \Gamma^d(p^d_x) = 1] = 1/5\\
FPR(a) &= Pr[Y = 0 | \Gamma^a(p^a_x) = 1] = 1 - Pr[Y = 1 | \Gamma^a(p^a_x) = 1] = 1/3~.~
\end{align*} 

\paragraph{Questions to ask}
Question to ask here include understanding if we can (a) reprove the results of Kleinberg, Mullainathan, and Raghavan with this scheme, and (b) generalize their (impossibility) results to other settings of fairness beyond binary classification.
